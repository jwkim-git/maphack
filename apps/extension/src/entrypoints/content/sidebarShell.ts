...
function ensureGlobalStyle(documentRef: Document): void {
  ...
  style.textContent = [
    ...
    "0%,100%{transform:translate(-50%,-50%) translate3d(0,7%,0)}",
    "50%{transform:translate(-50%,-50%) translate3d(0,-7%,0)}}",
    `#${SIDEBAR_OPEN_BUTTON_ID}:hover{scale:1.05}`,
    `#${SIDEBAR_OPEN_BUTTON_ID}:hover svg{transform:scale(1.1) rotate(5deg)}`
  ].join("");
